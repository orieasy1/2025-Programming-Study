...
def start_recording():
    ...
    with lock:
        ...
        # 이벤트 시간 기준으로 과거 3초 
        recent_frames = list(pre_event_frames)
        frames_to_write = recent_frames[-(frame_rate * 3):]  # 최근 3초

        for ts, frame in frames_to_write:
...
def generate_frames():
    global is_recording, motion_start_time, post_record_counter, post_recording_end_time

    while True:
        ret, frame = cap.read()
        if not ret:
            print("[INFO] 영상 재생이 끝났습니다. 서버를 종료합니다.")
            stop_recording()
            os._exit(0)
        ...
        if change_detected:
            ...
            # 변화가 없을 때만 후처리 타이머 시작
            if is_recording and post_recording_end_time is None:
                post_recording_end_time = now + 2  # ← 정확히 2초 후

            if is_recording and post_recording_end_time is not None:
                if now >= post_recording_end_time:
                    stop_recording()
                    post_recording_end_time = None

        # 스트리밍을 위한 프레임 인코딩 및 전송
